...
# 윈도우와 공유할 폴더 생성
    def smb_share_folder_make(self):
        self.smb_sever_share_folder = input("공유할 폴더 이름을 입력해주세요. ex)/mnt/win_share : ")
        cmd = f"mkdir {self.smb_server_share_folder}" 
        return cmd
...
#공유 파일 권한 조정
    def smb_share_folder_authority(self):
        cmd = f"""
chmod 777 {self.smb_sever_share_folder}
chown {self.smb_id}:{self.smb_id} {self.smb_sever_share_folder}        
        """
        return cmd
...
[{self.share_name}]
    ...
    path = {self.smb_sever_share_folder}
...
def smb_client_login(self) :
    ...
    self.smb_password = input("smb password를 입력하세요. ex)smb : ")
